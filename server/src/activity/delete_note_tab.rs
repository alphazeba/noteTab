...
#[delete("/tab/<key>")]
pub fn delete_note_tab(key: NoteTabKey, injectables_state: &State<Injectables>) -> Json<DeleteNoteTabOutput> {
    let ioio = injectables_state.inner().get_io();
    let success = match ioio.delete(key.get().to_string()) {
        Ok(_) => true,
        Err(e) => {
